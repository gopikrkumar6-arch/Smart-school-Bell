    ...
    >
      ...
      {!runsToday && (
        <div className="absolute top-1 right-12">
          <span className="text-[7px] font-black text-slate-300 uppercase tracking-widest">Not Today</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[8px] font-black bg-indigo-50 text-indigo-400 px-1.5 py-0.5 rounded-md uppercase tracking-tighter">🔔 {period.ringDuration || 10}s</span>
            </div>
            <div className="flex gap-0.5 mt-0.5">
               {DAYS_SHORT.map((d, i) => (
                 <span key={i} className={`text-[6px] font-black ${period.repeatDays.includes(i) ? 'text-indigo-400' : 'text-slate-200'}`}>
                   {d}
                 </span>
               ))}
            </div>
        ...
        <div className="flex items-center space-x-2">
          <button 
            ...
            className={`w-10 h-6 rounded-full transition-all duration-300 relative flex items-center shadow-inner active:scale-110 ${
              period.isActive ? (isActive ? 'bg-indigo-500' : isNext ? 'bg-emerald-500' : 'bg-indigo-400') : 'bg-slate-200'
            }`}
          >
            <div className={`w-4 h-4 bg-white rounded-full absolute transition-all duration-300 shadow-sm transform ${
              period.isActive ? 'translate-x-[20px]' : 'translate-x-1'
            }`} />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-3 text-slate-400">
        <div className="flex flex-col items-start">
          <span className="text-[7px] font-black uppercase tracking-tighter opacity-40 mb-0.5">Start</span>
        ...
        <div className="flex-1 flex items-center justify-center relative h-6 group">
          <div className="h-2 w-full bg-slate-100/60 rounded-full overflow-hidden absolute border border-slate-50 shadow-inner">
            <div 
              className={`h-full transition-all duration-1000 ease-linear rounded-r-full relative ${
                isActive && runsToday ? 'bg-amber-400 sand-texture' : 'bg-transparent'
              }`}
          ...
          <div className={`z-10 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border transition-all duration-300 ${
            isActive && runsToday
        ...
        <div className="flex flex-col items-end">
          <span className="text-[7px] font-black uppercase tracking-tighter opacity-40 mb-0.5 text-right">End</span>
          <span className={`text-lg font-black transition-colors duration-300 tabular-nums leading-none ${isActive ? 'text-indigo-600' : isNext ? 'text-emerald-600' : 'text-slate-900'}`}>
